...
function convertAttendance() {
  ...
  for (var r = 1; r < attendanceValues.length; r++) {
    ...
    if (!name) continue;
    var studentId = attendanceValues[r][1];
    for (var c = 2; c < attendanceValues[r].length; c++) {
      var mark = attendanceValues[r][c];
      var sessionId = sessionIds[c - 2];
      if (mark !== '' && mark != null && sessionId) {
        participants.push([sanitizeForSheet_(name), sanitizeForSheet_(studentId), sanitizeForSheet_(sessionId)]);
      }
    }
  }
  ...
  if (lastRow > 1) {
    participantSheet.getRange(2, 1, lastRow - 1, 3).clearContent();
  }
  if (participants.length > 0) {
    participantSheet.getRange(2, 1, participants.length, 3).setValues(participants);
  }
}
...
function ensureParticipantHeaders_(sheet) {
  sheet.getRange(1, 1, 1, 3).setValues([['성명', '학생ID', '세션ID']]);
}
